...
export default function Accomplishments({ data, setData, onBack, onNext, isQuickEdit, onReturnToDashboard, isFieldSupported, currentTemplateName, isMobile }) {

    // 1. Direct Read & Migration (No State)
    const getAccomplishments = () => {
        if (!data.accomplishments) return [{ name: "", description: "" }];

        if (Array.isArray(data.accomplishments) && data.accomplishments.length > 0) {
            return data.accomplishments.map(ach =>
                typeof ach === 'string' ? { name: ach, description: "" } : ach
            );
        }

        if (typeof data.accomplishments === 'string' && data.accomplishments.trim() !== "") {
            const isHTML = /<[a-z][\s\S]*>/i.test(data.accomplishments);
            if (isHTML || data.accomplishments.length > 100) {
                return [{ name: "Accomplishment", description: data.accomplishments }];
            }
            return [{ name: data.accomplishments, description: "" }];
        }
    ...
    const handleSearch = async (term) => {
        ...
        try {
            ...
            setTimeout(() => {
                setAiSuggestions([
                    "Employee of the Month for three consecutive months",
                    "Exceeded sales targets by 25% in Q3",
                    "Reduced operational costs by 15% through workflow optimization",
                    "Awarded 'Best Innovation' at the annual company hackathon",
                    "Successfully managed a project with a budget of over $1 million",
                    "Implemented a new CRM system that improved lead conversion by 10%",
                    "Published 3 research papers in international journals",
                    "Mentored 5 junior developers who were later promoted",
                    "Volunteered 100+ hours for local community projects",
    ...
    const handleAddSuggestion = (text) => {
        setData(prev => ({
            ...prev,
            accomplishments: [...accomplishments, { name: text, description: "" }]
        }));
    };
    ...
    const addAccomplishment = () => {
        setData(prev => ({
            ...prev,
            accomplishments: [...accomplishments, { name: "", description: "" }]
        }));
    };

    const updateAccomplishment = (index, field, value) => {
        if (errors[index]) setErrors(prev => ({ ...prev, [index]: null }));
        const updated = accomplishments.map((item, i) =>
            i === index ? { ...item, [field]: value } : item
        );
        setData(prev => ({ ...prev, accomplishments: updated }));
    };
    ...
    const removeAccomplishment = (index) => {
        ...
        setData(prev => ({
            ...prev,
            accomplishments: updated.length > 0 ? updated : [{ name: "", description: "" }]
        }));
    };

    const moveAccomplishment = (index, direction) => {
        if (direction === 'up' && index === 0) return;
        if (direction === 'down' && index === accomplishments.length - 1) return;
    ...
    const handleNext = () => {
        ...
        accomplishments.forEach((item, index) => {
            if (!item.name?.trim()) {
                newErrors[index] = "Accomplishment Title is required.";
            }
        });
        ...
        <div className="animate-fade" style={{ maxWidth: '1200px', margin: '0 auto' }}>
            ...
            <div className="ai-header" style={{ marginBottom: isMobile ? '24px' : '40px', padding: isMobile ? '0 16px' : '0' }}>
                ...
                <div style={{ display: 'flex', alignItems: isMobile ? 'flex-start' : 'center', gap: '12px', flexDirection: isMobile ? 'column' : 'row' }}>
                    <h1 className="form-title" style={{ fontSize: isMobile ? '24px' : '28px', marginBottom: '4px', lineHeight: 1.2 }}>Tell us about your accomplishments</h1>
                    <CompatibilityWarning isSupported={isFieldSupported('accomplishments')} templateName={currentTemplateName} />
                </div>
                <p className="form-subtitle" style={{ fontSize: isMobile ? '14px' : '16px', color: '#64748b', marginTop: '4px' }}>Highlight your biggest wins and professional milestones.</p>
            </div>
            ...
            <div style={{
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '24px', order: isMobile ? 1 : 2 }}>
                        ...
                        <div key={index} className="form-section-card" style={{
                            ...
                            {!isMobile && (
                                <div style={{ position: 'absolute', left: '-40px', top: '24px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                    <button onClick={() => moveAccomplishment(index, 'up')} disabled={index === 0} style={{ opacity: index === 0 ? 0.3 : 1, color: '#94a3b8', background: 'none', border: 'none', cursor: index === 0 ? 'default' : 'pointer' }}><ChevronUp size={20} /></button>
                                    <button onClick={() => moveAccomplishment(index, 'down')} disabled={index === accomplishments.length - 1} style={{ opacity: index === accomplishments.length - 1 ? 0.3 : 1, color: '#94a3b8', background: 'none', border: 'none', cursor: index === accomplishments.length - 1 ? 'default' : 'pointer' }}><ChevronDown size={20} /></button>
                                </div>
                            )}
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '24px', gap: '12px' }}>
                                <div style={{ flex: 1 }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
                                        <label className="label" style={{ margin: 0, fontWeight: 700, fontSize: '14px', color: '#1e293b' }}>Title <span style={{ color: '#ef4444' }}>*</span></label>
                                        <CompatibilityWarning isSupported={isFieldSupported('accomplishments.name')} templateName={currentTemplateName} />
                                    </div>
                                    <input
                                        className="zety-input"
                                        value={ach.name || ''}
                                        onChange={(e) => updateAccomplishment(index, 'name', e.target.value)}
                            ...
                            <div>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
                                    <label className="label" style={{ margin: 0, fontWeight: 600 }}>Description (Optional)</label>
                                    <CompatibilityWarning isSupported={isFieldSupported('accomplishments.description')} templateName={currentTemplateName} />
                                </div>
                    ...
                    >
                        <Plus size={18} /> Add Another Accomplishment
                    </button>
